...
- Dado o crescimento do número de estudos sobre temas de gênero, nota-se certa tendência a substituir o termo "sexo" pela terminologia mais atualizada de "gênero".
...
- A \structure{razão de sexos} (também conhecida com razão ou índice de masculinidad) para o total $(RS_t)$ da população:

\footnotesize

$$
\color{black}{\frac{(\mbox{número de pessoas do sexo masculino})}{(\mbox{número de pessoas do sexo feminino})}\times 100.}
$$
\normalsize

- Na notação introduzida, temos:

$$
RS_t = \frac{P^H_{0^{+}}}{P^M_{0^{+}}} \times 100.
$$

\framebreak

Para os dados do Censo 2014 de Angola, temos que 

- $P^H_{0^{+}} = 12.499.041$;
- $P^M_{0^{+}} = 13.289.983$;

$$
RS_t = \frac{12.499.041}{13.289.983}\times 100 = 94,05.
$$
...
+ Já o caso de **Cabo Verde** nos anos 70, com uma $RS_t$ de 107,7 (critério *de iure*^[critério de contagem pela sua residência habitual e não pelo local de presença na data do censo (critério *de facto*).]), estaria relacionada com a entrada maciça de mão de obra angolana e nos anos mais recentes à diáspora que tem caracterizado o país.

## $RS_t$: exemplos  {.allowframebreaks}

Também há o caso de países com práticas de valoração do sexo masculino em detrimento das mulheres.

- Seria o caso da China, onde estas práticas, impactariam mais a razão de sexos entre crianças e que explicaria os altos valores para a $RS_t$. 
...
O **Gráfico 6.4** apresenta as $RS_t$ estimada pela Divisão de População da ONU para os **PALOP** (Países Africanos de Língua Oficial Portuguesa), total mundial e para os países mais desenvolvidos e em desenvolvimento (1975-2015). 
...
# Razão de Sexos ao Nascer {.allowframebreaks}
...
Valores fora do intervalo de 103 a 107 possuem qualidade deficiente ou são resultado, principalmente de intervenções de ordem:
...
- **Sanitária:** por corresponder ao sexo masculino maiores riscos de mortalidade, inclusive antes de nascer, avanços médicos que levam a gravidez de risco até um nascido vivo, produziriam, proporcionalmente, mais nascimentos masculinos e $RS_0$ maiores.
...
- Entretanto, o único país no continente com uma $RS_0$ definitivamente atípica é o **Suriname** $(RS_0 = 108)$.

\framebreak

No caso dos **países asiáticos** os valores parecem atípicos; 

- por um lado, na **Coreia do Sul**, e em menor medida, no **Japão**, parece haver uma tendência de aproximação do indicador à marca de 105;
- enquanto há um claro excesso de nascimentos masculinos na **China** e na **Índia**. 
    <!-- + O valor de Macau parece normal, mas é difícil de saber por certo, devido ao número reduzido de nascimentos. -->

\framebreak

O comportamento da $RS_0$ pode ser retratado também considerando sua tendência por idade da mãe ou ainda pela ordem do nascimento. 
...
- O **Gráfico 6.8** descreve duas situações que ilustram bem situações atípicas dentro dos países PALOP na virada do século XXI: Guiné-Bissau e Moçambique, países que, além de __sabidas dificuldades com a qualidade dos dados__, experimentaram no passado recentes __turbulências sociais__ de variada intensidade.
...
- Tendência monotonamente decrescente com a idade até atingir o equilíbrio entre sexos, o que deve ocorrer depois da idade 25-30, dados os níveis de mortalidade do século XXI;
