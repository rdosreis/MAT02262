...
subtitle: "Crescimento e distribuição da população"
fontsize: 10pt
author: |
...
- É importante ressaltar como se caracteriza o __ritmo de crescimento__ de uma população.

\framebreak

O ponto de partida para qualquer medição do crescimento é a comparação do **tamanho de uma população** ou subpopulação **em dois momentos do tempo**:

$$
t\quad \mbox{e}\quad t + \Delta t.
$$
...
- A contraparte das fórmulas anteriores no cálculo de $P(t + \Delta t)$ a partir de $P(t)$, usando $r$, é a seguinte:
...
- Note que no caso da taxa anual, temos $\Delta t = 1$, e portanto,

$$
r = \frac{P(t + 1)}{P(t)} - 1 = \frac{P(t + 1) - P(t)}{P(t)},
$$
geralmente expressa em termos percentuais. E consequentemente, temos que
...
+ Por exemplo, se espera que a população da América Latina aumentará de 634 milhões em 2015 para 784 milhões em 2050. Se os valores de 634 e 784 são substituídos para $P(t)$ e $P(t+ \Delta t)$ nas fórmulas $r$, com $\Delta t = 35$, o resultado para a taxa anual é 0,006086 (0,6086%) e para a taxa instantânea 0,006067.
...
## Tempo de duplicação {.allowframebreaks}

- Para ajudar a visualizar as implicações de uma determinada taxa de crescimento a mais **longo prazo** às vezes se calcula o chamado \structure{tempo de duplicação}.
- Ou seja, o tempo necessário para que uma população que cresce com certa taxa alcance o dobro do seu tamanho inicial. 

\framebreak

- Como existem efeitos de acumulação (crescimento sobre crescimento), o tempo que uma população que cresce a 2% por ano leva para duplicar não é 50 anos, mas só 35 anos.
- Uma população que cresce a 4% por ano duplica em apenas 17,7 anos.

- As respectivas fórmulas são as seguintes:
...
Em termos mais formais, temos a seguinte expressão para \structure{Fórmula Básica da Contabilidade Demográfica}:
...
1. Uma ou mais quantidades que constam da fórmula podem ter sido medidas incorretamente; ou
2. As unidades territoriais correspondentes às populações em $t$ e $t+n$ podem não ser as mesmas, por exemplo porque se trata de um município ou uma província cujos limites foram modificados durante o período.
...
## O conceito de coorte {.allowframebreaks}
