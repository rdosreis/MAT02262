\usepackage{longtable}
\usepackage{multirow}
\usepackage{xcolor}
\usepackage{mathtools}
\usepackage{tikz}
\usepackage{amsmath}
\usepackage[brazil]{babel}   
\usepackage[utf8]{inputenc}
\usepackage[most]{tcolorbox}
\usepackage{pgothic}

\newtcolorbox{mybox}[2][]{%
    colback=bg, 
    colframe=blue!75!black,
    fonttitle=\bfseries,
    coltitle=blue!75!black, 
    colbacktitle=bg,
    enhanced,
    attach boxed title to top left={yshift=-1.2mm, xshift=2mm},
  title=#2,
  #1}

\setbeamercolor{math text}{fg=black!15!red}

\newcommand\beamermathcolor[1]{\color{#1}\setbeamercolor{math text}{fg=#1}}
\newcommand{\Comb}[2]{{}_{#1}{C}_{#2}}
\newcommand{\specialcell}[2][c]{%
\begin{tabular}[#1]{@{}c@{}}#2\end{tabular}}

\newcount\tmpnum
\def\tallymarks#1{\leavevmode \lower1bp\vbox to9bp{}%
   \tmpnum=#1
   \loop \ifnum\tmpnum<5 \kern1bp \tallynum\tmpnum \else \tallyV \fi
         \advance\tmpnum by-5
         \ifnum\tmpnum>0 \repeat
}
\def\tallynum#1{\bgroup\tmpnum=#1\relax
   \loop \ifnum\tmpnum>0
         \kern1bp \tallyI \kern1bp
         \advance\tmpnum by-1
         \repeat
   \egroup
}
\def\tallyI{\pdfliteral{q .5 w 0 -1 m 0 8 l S Q}}
\def\tallyV{\kern1bp\pdfliteral{q .5 w -1 0 m 9 7 l S Q}\tallynum4\kern1bp }

\newcommand\blfootnote[1]{%
  \begingroup
  \renewcommand\thefootnote{}\footnote{#1}%
  \addtocounter{footnote}{-1}%
  \endgroup
}

\setbeamertemplate{frametitle continuation}{}
\setbeamertemplate{items}[default] 
\setbeamertemplate{enumerate items}[default]
\setbeamertemplate{footline}{}
\setbeamercolor{alerted text}{fg=purple}
\titlegraphic{\hfill\includegraphics[height=1cm]{logos/ime.png}}

\def\Corr{\mbox{Corr}\,}
\def\Cov{\mbox{Cov}\,}
\def\Var{\mbox{Var}\,}
\def\E{\mbox{E}\,}
\def\logit{\mbox{logit}\,}
\definecolor{darkpastelblue}{rgb}{0.47, 0.62, 0.8}
\definecolor{darkpastelgreen}{rgb}{0.01, 0.75, 0.24}
\definecolor{darkpastelpurple}{rgb}{0.59, 0.44, 0.84}
\definecolor{darkpastelred}{rgb}{0.76, 0.23, 0.13}
\definecolor{darkviolet}{rgb}{0.59, 0, 0.84}
\definecolor{darkgreen}{rgb}{0, 0.512, 0}
\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}