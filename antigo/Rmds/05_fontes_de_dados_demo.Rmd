...
- Como brevemente discutido anteriormente, os dados mais usados pelos demógrafos dizem respeito aos \structure{fatos vitais:} __uniões__, __nascimentos__, __óbitos__ e __movimentos da população__.
...
1. o registro oficial dos fatos vitais e demográficos \alert{(óbito, nascimento vivo, casamento, divórcio, adoção, migração, etc.)};
...
- Mas o que na prática existe é algo diverso e a seqüência acima raramente prevalece.
...
- __No Brasil:__
    + __Órgão responsável:__ Instituto Brasileiro de Geografia e Estatística (IBGE).
    + __Periodicidade:__ decenal (com algumas exceções: Censos 1991, 2022). Veja aqui sobre a história dos censos demográficos no Brasil: \alert{\underline{\href{https://agenciadenoticias.ibge.gov.br/agencia-noticias/2012-agencia-de-noticias/noticias/33495-em-150-anos-conheca-a-historia-que-o-censo-conta}{Em 150 anos, conheça a história que o Censo conta}}}.
    + __Simultaneidade:__ Data de referência do Censo de 2022 --- a investigação das características dos domicílios e das pessoas neles residentes terá como data de referência a meia-noite do dia 31 de julho para 1º de agosto de 2022. Portanto, de acordo com este critério, as pessoas nascidas após esta data serão excluídas do Censo, pois não farão parte do conjunto de moradores do domicílio na data de referência. Já as pessoas falecidas após esta data serão incluídas, pois eram moradoras do domicílio na data de referência.
...
1. Pré-recenseamento
    ...
    + Recrutamento e treinamento da equipe de agentes censitários.
2. Recenseamento:
    + Aplicação dos questionários e supervisão técnica.
3. Pós-Recenseamento:
...
- O \alert{\underline{\href{https://censo2022.ibge.gov.br/np_download/censo2022/questionario_basico_completo_CD2022_atualizado_20220906.pdf}{Questionário Básico}}} da pesquisa conta com __26 questões__ e investiga as \structure{principais características do domicílio e dos moradores}. 
- Além disso, uma parcela dos domicílios é selecionada para responder ao \alert{\underline{\href{https://censo2022.ibge.gov.br/np_download/censo2022/questionario_amostra_completo_CD2022_atualizado_20220906.pdf}{questionário da Amostra}}}, que conta com __77 questões__. 
...
+ No Censo 2022, foram (serão) investigados cerca de 75 milhões de domicílios particulares permanentes do País. 
    + O Questionário da Amostra foi (será) aplicado em aproximadamente __11%__ desse contingente, ou seja, cerca de 8,5 milhões de domicílios.

::: {.block}    

### Por que uma amostra?

1. Para se disponibilizar determinações informações mais rapidamente;
...
É a menor unidade de disseminação dos dados do conjunto Universo.
...
Uma unidade geográfica, formada por um agrupamento de setores censitários, para a aplicação dos procedimentos de calibração das estimativas com as informações conhecidas para a população como um todo.
...
Em pesquisas por amostragem probabilística, cada unidade selecionada na amostra representa, também, outras unidades que fazem parte da população- alvo.
...
- Para o Censo de 2022: \alert{\underline{\href{https://censo2022.ibge.gov.br/etapas/divulgacao-dos-resultados.html}{Divulgação dos resultados (Prévia)}}}
- Para os censos em geral o IBGE realiza publicações, como por exemplo a publicação \alert{\underline{\href{https://biblioteca.ibge.gov.br/visualizacao/periodicos/93/cd_2010_caracteristicas_populacao_domicilios.pdf}{Censo demográfico 2010: características da população e dos domicílios - resultados do universo}}}
- Também é possível consultar os resultados através de tabelas pelo \alert{\underline{\href{https://sidra.ibge.gov.br/home/abate/brasil}{Sistema IBGE de Recuperação Automática - SIDRA}}}
...
- Explore os resultados do Censo de 2010 no site do SIDRA; apresente no Fórum Geral do Moodle os destaques em termos das característica da população brasileira que você encontrou.
...
- Fontes de dados: registro civil e PNAD.

```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='60%', paged.print=FALSE}

knitr::include_graphics(here::here('imagens', 'registro_civil.png'))
...
knitr::include_graphics(here::here('imagens', 'Statistically-Insignificant-barras02.jpg'))
